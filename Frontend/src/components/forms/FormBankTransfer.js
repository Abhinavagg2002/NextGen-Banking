...
const FormBankTransfer = () => {
  const subheading = "Money Transfer Form"
  const description = ""
  ...
  const transfer = async(e) => {
    ...
    if(parsedData === null) {
      ...
      const response = await fetch('http://localhost:4000/transfer', {
            method: 'PATCH',
            body: JSON.stringify({id, amount, password, date,account_no}),
        ...
        <TextColumn textOnLeft={textOnLeft}>
          <TextContent>
            ...
            <Form onSubmit={transfer}>
              <Input type="text" name="text" placeholder="Account Number of payee" value={account_no} onChange={e => setaccount(e.target.value)}/>
              <Input type="number" name="number" placeholder="Amount" value={amount} onChange={e => setamount(e.target.value)}/>
...
export default FormBankTransfer
