...
const FormBankWithdrawl = () => {
  const subheading = "Money Withdrawl Form"
  ...
  const date = formatDateToIndianFormat(currentDate);
      ...
      const withdrawl = async (e) => {
        ...
        if(parsedData === null) {
          ...
          const response = await fetch('http://localhost:4000/withdrawl', {
                method: 'PATCH',
                body: JSON.stringify({id, amount, password, date}),
                headers: {'Content-Type':'application/json', Accept: "application/json"},
                credentials: 'include',
          })
          if(response.ok){
            alert('amount withdrawl successfull.')
            setDash(true)
          }else {
            try {
...
export default FormBankWithdrawl;
